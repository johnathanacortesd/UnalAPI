...
# ======================================
# Análisis con IA (v9.3 - MÁXIMA PRECISIÓN)
# ======================================
...
async def run_full_process_async(dossier_file, region_file, internet_file, brand_name):
    ...
    # ==========================================================
    # CORRECCIÓN CRÍTICA (v9.3): Lógica de post-procesamiento robusta
    # ==========================================================
    with st.spinner("Finalizando y ensamblando el reporte..."):
        ai_col_keys = [key_map["tonoai"], key_map["temaespecifico"], key_map["temageneral"]]
        for col_key in ai_col_keys:
            # Paso 1: Garantizar que la columna exista, si no, crearla vacía.
            if col_key not in df_all.columns:
                df_all[col_key] = ''
            
            # Paso 2: Rellenar cualquier NaN restante con una cadena vacía.
            df_all[col_key] = df_all[col_key].fillna('')
            
            # Paso 3: Sobrescribir las filas duplicadas con la etiqueta 'Duplicada'.
            df_all.loc[df_all['is_duplicate'] == True, col_key] = 'Duplicada'
...
def main():
    ...
    st.markdown("<hr><div style='text-align:center;color:#666;font-size:0.9rem;'><p>Sistema de Análisis de Noticias v9.3 (Máxima Precisión) | Adaptado para la Universidad Nacional</p></div>", unsafe_allow_html=True)
