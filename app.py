...
def limpiar_texto_simple(tema: str) -> str:
    ...
    return tema[0].upper() + tema[1:] if tema else ""
...
def agrupar_noticias_por_similitud_inteligente(rows: List[Dict], key_map: Dict[str, str]) -> Dict[int, List[int]]:
    if not rows: return {}
    textos_comparacion = [f"{clean_title_for_output(row.get(key_map.get('titulo'), ''))} {clean_title_for_output(row.get(key_map.get('titulo'), ''))}" for row in rows]
    embs, valid_indices = [], []
...
# ======================================
def _crear_tema_desde_titulo_deterministico(titulo: str, marca: str, aliases: List[str]) -> str:
    """Fallback 100% determinístico sin IA para crear un tema desde el título."""
    palabras_prohibidas = {
        "en", "de", "del", "la", "el", "y", "o", "con", "sin", "por", "para", "sobre", "un", "una", "los", "las", "al", "a",
        "ante", "bajo", "cabe", "contra", "desde", "durante", "entre", "hacia", "hasta", "mediante", "segun", "so", "tras",
        "qué", "es", "son", "fue", "ser", "estar",
        marca.lower(), *[a.lower() for a in aliases]
    }
    titulo_limpio = normalize_title_for_comparison(clean_title_for_output(titulo))
    palabras = [p for p in titulo_limpio.split() if p not in palabras_prohibidas and len(p) > 2]
    tema = " ".join(palabras[:4])
    return limpiar_texto_simple(tema) or "Tema por Título"

class ClasificadorIA:
    def __init__(self, marca: str, aliases: List[str]):
        self.marca = marca
        self.aliases = aliases or []
        self.palabras_prohibidas_tema = { self.marca.lower(), *[a.lower() for a in self.aliases], "universidad", "nacional", "colombia"}
    ...
    async def _analizar_grupo_async(self, titulo: str, resumen: str, semaphore: asyncio.Semaphore) -> Dict[str, str]:
        async with semaphore:
            ...
            contexto_resumen = '. '.join([o.strip() for o in re.split(r'[.!?]+', resumen)[:2] if o.strip()])
            
            prompt = f"""Tu tarea es analizar una noticia sobre '{self.marca}' y devolver un análisis en formato JSON.

**Instrucción principal: EXTRAER LA FRASE CLAVE DEL TÍTULO.** No inventes, no interpretes, no generalices.

**TÍTULO DE LA NOTICIA:** "{titulo_limpio}"

**Pasos:**
1.  **TEMA**: Lee el título y extrae la frase principal (de 2 a 5 palabras) que describe el evento o sujeto de la noticia. Debe ser casi una copia directa de una parte del título.
2.  **TONO**: Clasifica el tono general de la noticia hacia la marca (Positivo, Negativo, Neutro).

**Ejemplos de extracción CORRECTA de tema:**
- Título: "UNAL inaugura el nuevo edificio de artes en el campus" -> **TEMA CORRECTO:** "Inauguración nuevo edificio de artes"
- Título: "Estudiantes de la Nacional protestan por alza en matrículas" -> **TEMA CORRECTO:** "Protesta por alza de matrículas"
- Título: "Convenio entre la U. Nacional y Ecopetrol para investigar biocombustibles" -> **TEMA CORRECTO:** "Convenio para investigar biocombustibles"

**Ejemplos de temas INCORRECTOS (¡EVITAR!):**
- "Educación Superior" (demasiado genérico)
- "Anuncios UNAL" (no describe la noticia)
- "Ciencia y Tecnología" (demasiado amplio)

**CONTEXTO ADICIONAL (para el tono):** {contexto_resumen[:MAX_TOKENS_PROMPT_TXT]}
...
Responde **únicamente** con el objeto JSON: {{"tono": "...", "tema": "..."}}"""
            ...
            try:
                ...
                # Validación final y fallback determinístico
                palabras_tema = set(tema_bruto.lower().split())
                if not tema_bruto or tema_bruto == "Sin tema" or not any(p not in self.palabras_prohibidas_tema for p in palabras_tema):
                    tema_final = _crear_tema_desde_titulo_deterministico(titulo_limpio, self.marca, self.aliases)
                else:
                ...
                return {"tono": "Neutro", "tema": _crear_tema_desde_titulo_deterministico(titulo_limpio, self.marca, self.aliases)}
...
async def consolidar_temas(temas: List[str], p_bar) -> List[str]:
    ...
    tema_counts = Counter(t for t in temas if t not in ["Sin tema", "Error de Análisis", "Tema por Título"])
...
def _append_rows_to_sheet(sheet, rows_data, key_map, include_ai_columns):
    ...
    for row_data in rows_data:
        ...
        for col_idx, header in enumerate(final_order, 1):
            ...
            else: cell_value = str(val) if val is not None else ""
...
async def run_full_process_async(dossier_file, region_file, internet_file, brand_name, brand_aliases):
    try:
        ...
        st.error("❌ Error: OPENAI_API_KEY no encontrado en los Secrets de Streamlit."); st.stop()
