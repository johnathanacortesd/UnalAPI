...
# ======================================
# Análisis con IA (v9.2 - MÁXIMA PRECISIÓN)
# ======================================
...
async def run_full_process_async(dossier_file, region_file, internet_file, brand_name):
    ...
    if df_unal_to_analyze.empty:
        ...
        with st.status(f"🧠 **Paso 2/3:** Analizando Tono y Tema para {len(df_unal_to_analyze)} noticias de '{brand_name}'...", expanded=True) as s:
            ...
            # ==========================================================
            # CORRECCIÓN CRÍTICA (v9.2): Usar MERGE en lugar de UPDATE
            # ==========================================================
            ...
            # Rellenar las columnas nuevas en las filas que no fueron analizadas de forma segura
            for col_key in [key_map["tonoai"], key_map["temaespecifico"], key_map["temageneral"]]:
                # Primero, llena todos los NaN (de filas no-UNAL) con una cadena vacía
                df_all[col_key] = df_all[col_key].fillna('')
                # Segundo, sobrescribe las filas duplicadas con la etiqueta 'Duplicada'
...
def main():
    ...
    st.markdown("<hr><div style='text-align:center;color:#666;font-size:0.9rem;'><p>Sistema de Análisis de Noticias v9.2 (Máxima Precisión) | Adaptado para la Universidad Nacional</p></div>", unsafe_allow_html=True)
